...
Levels.setURL("mongodb+srv://YokkoEddy23:zjmbvgwr52@cluster0.qh05pl9.mongodb.net/?retryWrites=true&w=majority");
...
module.exports = async (Icaa, m, commands, chatUpdate, store) => {
    try {
        let {
            type,
            isGroup,
        ...
        const metadata = isGroup ? await Icaa.groupMetadata(from) : {};
        const pushname = m.pushName; //|| 'NO name'
        const participants = isGroup ? metadata.participants : [sender];
        const groupAdmin = isGroup ?
            participants.filter((v) => v.admin !== null).map((v) => v.id) :
            [];
        const botNumber = await Icaa.decodeJid(Icaa.user.id);
        const isBotAdmin = m.isGroup ? groupAdmin.includes(Icaa.user?.jid) : false;
        const isAdmin = isGroup ? groupAdmin.includes(sender) : false;
        const isCreator = [botNumber, ...global.owner]
            .map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net")
            .includes(m.sender);
        const isOwner = global.owner.includes(m.sender);
        global.suppL = 'https://cutt.ly/RungkadsBotSupport';
        ...
        if (checkdata) {
            ...
            if (m.isGroup && mongoschema == "true") {
                linkgce = await Icaa.groupInviteCode(from);
                if (budy.includes(`https://chat.whatsapp.com/${linkgce}`)) {
                    ...
                    kice = m.sender;
                    await Icaa.groupParticipantsUpdate(m.from, [kice], "remove");
        ...
        if (body.startsWith(prefix) && !icmd) {
            let Icaatext = `No such command programmed *${pushname}* senpai! Type *${prefix}help* or press the button below to get my full command list!\n`;
        ...
        if (
            ...
            await Icaa.sendMessage(m.from, reactionMess).then(() => {
                return Icaa.sendMessage(m.from, buttonmess, {
                    react: "🍁",
                    quoted: m
                });
            });
        ...
        if (cmd.react) {
            ...
            await Icaa.sendMessage(m.from, reactm);
        }
