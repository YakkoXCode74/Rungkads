module.exports = {
    ...
    start: async (
      ...
      { text, prefix, isBotAdmin, isAdmin, mentionByTag, args, pushName, mime, quoted }
        ...
        if (args[0] === 'close'){
            await Icaa.groupSettingUpdate(m.from, 'announcement').then((res) => m.reply(`Group has been closed!`))
            } else if (args[0] === 'open'){
            await Icaa.groupSettingUpdate(m.from, 'not_announcement').then((res) => m.reply(`Group has been opened!`))
            } else {
            let buttons = [
