module.exports = {
  ...
  react: "🍁",
  category: "Core",
  start: async (Icaa, m, { pushName, args, prefix }) => {
    if (!args[0])
    ...
    for (let i = 0; i < randomxx; i++) {
        ...
        var anu = await Icaa.onWhatsApp(`${number0}${i}${number1}@s.whatsapp.net`);
        var anuu = anu.length !== 0 ? anu : false
        try {
        try {
        var anu1 = await Icaa.fetchStatus(anu[0].jid)
        } catch {
        var anu1 = '401'
        }
        ...
        await Icaa.sendMessage(m.from, { text: `${nomerny} ${nobio} ${nowhatsapp}` }, { quoted: m });
  },
};
