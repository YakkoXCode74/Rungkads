...
module.exports = { 
    ...
    start: async ( 
      ...
      if (modStatus=="false"&&!isCreator)  return Icaa.sendMessage(m.from, { text: 'Sorry, only my *Devs* and *Mods* can use this command !' }, { quoted: m });
...
//if (!banreason) return Icaa.sendMessage(m.from, { text: `Please provide the reason for ban.\n\n${prefix}ban spamming` }, { quoted: m });
      ...
      try { 
         mku.findOne({id:userId}).then(async (user) => {
            if (!user) {
              if (modStatus == "true" || ownerlist.includes(`${mentionedUser.split("@")[0]}`)) return Icaa.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} is a *Mod* and can't be banned !` , mentions: [mentionedUser]  }, { quoted: m });
              ...
              if (modStatus == "true" || ownerlist.includes(`${mentionedUser.split("@")[0]}`)) return Icaa.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} is a *Mod* and can't be banned !` , mentions: [mentionedUser]  }, { quoted: m });
                if (user.ban == "true") return Icaa.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} is already *Banned* !` , mentions: [mentionedUser]  }, { quoted: m });
           ...
           console.log(error)
           return Icaa.sendMessage(m.from, { text: `An internal error occurred while banning the user.` }, { quoted: m });
         });
      } catch (err) { 
        console.log(err);
        return Icaa.sendMessage(m.from, { text: `An internal error occurred while banning the user.` }, { quoted: m });
