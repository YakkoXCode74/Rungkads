         ...
         mku.findOne({id:userId}).then(async (user) => {
            if (!user) {
              return Icaa.sendMessage( 
                ...
                if (user.ban == "false") return Icaa.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} is not *Banned* !` , mentions: [mentionedUser]}, { quoted: m });
           ...
           console.log(error)
           return Icaa.sendMessage(m.from, { text: `An internal error occurred while Unbanning the user.` }, { quoted: m });
         });
      } catch (err) { 
        console.log(err);
        return Icaa.sendMessage(m.from, { text: `An internal error occurred while Unbanning the user.` }, { quoted: m });
