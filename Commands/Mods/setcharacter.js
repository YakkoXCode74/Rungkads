...
module.exports = { 
    ...
    start: async ( 
      ...
      let botNames = ['Rungkads MD', 'Power MD', 'Makima MD', 'Denji MD', 'Zero Two', 'Chika MD' , 'Icaa MD', 'Marin Kitagawa']
      let botLogos =[
      ...
      await mkchar.findOne({id:'1'}).then(async (charInfo) => {
        ...
        if (charInfo.seletedCharacter == charNum) {
          ...
          return m.reply(`Character number ${charNum} - ${botName} is already set as the default character.`);
            ...
            await mkchar.findOneAndUpdate({ id: '1'}, { $set: { seletedCharacter: charNum } }, { new: true }).then(async(res) => {
                await Icaa.sendMessage(m.from, { image: {url:botLogos[charNum]},caption: `Character number ${charNum} - ${botNames[charNum]} is now the default character.\n` }, { quoted: m })
            }).catch(error => {
        ...
        /*if (charNum == '0') {
            ...
            return await Icaa.sendMessage(m.from, { image: {url:botImage1},caption: `Character number ${charNum} - ${botName} is now the default character.\n` }, { quoted: m });
            ...
            return Icaa.sendMessage(m.from, { image: {url:botImage1},caption: `Character number ${charNum} - ${botName} is now the default character.\n` }, { quoted: m });
