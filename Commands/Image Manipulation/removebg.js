...
module.exports = {
  ...
  start: async (Icaa, m, { text, prefix, quoted, pushName, mime, body }) => {
	...
	if (/webp/.test(mime)) return m.reply (`Send/Reply Image With Caption *${prefix}removebg* to remove background of an image`);
        ...
        let qFile =await Icaa.downloadAndSaveMediaMessage(quoted, filename)
        ...
        await Icaa.sendMessage(m.from, {image: fs.readFileSync(outputFile),caption:`_Created by: *${botName}*_`}, {quoted: m}).then (async () => {
