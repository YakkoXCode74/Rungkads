...
module.exports = {
    ...
    let item = text
        if(!item) {
            return Icaa.sendMessage(m.from, { text:` 😕 Please specify an item to purchase. Use ${prefix}buy <item>` }, { quoted: m });
        }
        if(item === "woodenaxe") {
            if(balance.wallet < 250) {
                return Icaa.sendMessage(m.from, { text:` 😕 You don't have enough coins to purchase a wooden axe.` }, { quoted: m });
            }
            let deduct = await eco.deduct(m.sender, cara, 250);
            user.inventory.woodenaxe += 1;
            await user.save();
            Icaa.sendMessage(m.from, { text: `[ *💰PURCHASE RESULT💰* ]\n\n Successfully purchased a wooden axe!`}, { quoted: m });
        }
        else if(item === "stonepickaxe") {
            if(balance.wallet < 500) {
                return Icaa.sendMessage(m.from, { text:` 😕 You don't have enough coins to purchase a stone pickaxe.` }, { quoted: m });
            }
    ...
    if(user.inventory.goldenApple < 1) {
        return Icaa.sendMessage(m.from, { text:` 😕 You don't have a Golden Apple to purchase 100k Gold.` }, { quoted: m });
    }
...
return Icaa.sendMessage(m.from, { text: `😕 You don't have enough coins to purchase an iron pickaxe. `}, { quoted: m });
}
...
return Icaa.sendMessage(m.from, { text: `😕 You don't have enough coins to purchase a diamond pickaxe.` }, { quoted: m });
}
...
return Icaa.sendMessage(m.from, { text: `😕 Invalid item. Please use ${prefix}buy /item to purchase an item.` }, { quoted: m });
}
}
};
